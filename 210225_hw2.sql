...
-- SELECT문의 서브쿼리에서 WHERE E.MANAGER_ID = EMP_ID 여기서 사원엠플로이의 별칭을 붙여주지 않으면 매니저엠플로이의 매니저아이디와 사원아이디를 가져오기때문에 안돼... 
-- 별칭을 아무데나 붙이는게 아니었어. 
SELECT EMP_ID, EMP_NAME, (SELECT EMP_NAME FROM EMPLOYEE WHERE E.MANAGER_ID = EMP_ID), SALARY 
FROM EMPLOYEE E
...
FROM EMPLOYEE E
    ...
    JOIN JOB j on E.JOB_CODE=j.job_code -- using()일땐 별칭을 쓸 수 없다구!!!!!!~!~~~~~!
WHERE DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여') = '여'
    AND (SALARY+(SALARY*BONUS))*12 < (SELECT (AVG(SALARY+(SALARY*NVL(BONUS, 1)))*12) FROM EMPLOYEE WHERE E.JOB_CODE = JOB_CODE);
