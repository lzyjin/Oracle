...
WHERE JOB_CODE IN ('J5', 'J6', 'J7') 
      AND SALARY > ANY(SELECT SALARY FROM EMPLOYEE WHERE JOB_CODE = 'J4');
...
SELECT DEPT_CODE, EMP_NAME, SALARY, AVG(SALARY)
FROM EMPLOYEE
WHERE JOB_CODE NOT IN('J1', 'J2', 'J3') 
      AND SALARY > (SELECT AVG(SALARY) 
                    FROM EMPLOYEE 
                    GROUP BY DEPT_CODE)
GROUP BY DEPT_CODE, EMP_NAME, SALARY;

-- 다시 품 
SELECT DEPT_CODE, EMP_NAME, SALARY, JOB_CODE, FLOOR((SELECT AVG(SALARY) 
                                               FROM EMPLOYEE E2
                                               WHERE E1.DEPT_CODE = E2.DEPT_CODE)) AS "부서별 급여평균"
FROM EMPLOYEE E1
WHERE JOB_CODE NOT IN('J1', 'J2', 'J3') 
      AND SALARY > (SELECT AVG(SALARY) 
                    FROM EMPLOYEE E2
                    WHERE E1.DEPT_CODE = E2.DEPT_CODE)
GROUP BY DEPT_CODE, EMP_NAME, SALARY ,JOB_CODE;
